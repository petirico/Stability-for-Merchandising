...
class StabilityImageEditor:
    ...
    def feather_mask(self, mask: Image.Image, radius: int = 2, inner_clip: int = 1, gamma: float = 1.8) -> Image.Image:
        """
        ...
        """
        # If radius is non-positive, return the mask unchanged (no feathering)
        if radius <= 0:
            return mask.copy()
    ...
    # Removed face/background mask creation utilities per simplification request
...
def image_edit_background_and_face_mask(
    ...
    # Editable region: full image by default (protections applied below)
    combined = np.full((width if False else height, width), 255, dtype=np.uint8)

    # White polo protection removed per simplification request
    ...
    # Feather edges for smooth blending without grey halo (skip if radius <= 0)
    if feather_radius > 0:
    ...
    prompt = (
        ...
        "Background: Submerged ruins of an inverted glass cathedral rising from shallow neon-lit waters in post-AI Kyoto — stained glass shards refracting corrupted light patterns across the water; levitating data obelisks pulse faintly in the distance; moss-covered digital relics half-sunken at odd angles; floating koi-shaped surveillance drones drift silently above surface; distant mountain silhouettes barely visible through amber smog; soft bokeh blurs fractured reflections and collapsing spires. "
        "Lighting: surreal bioluminescent tones from underwater LED coral; faint uplight from submerged ruins; soft key light on subject’s face with cyan-magenta contrast fill; balanced exposure preserving shirt texture and emblem clarity. "
        "Lens: 85mm, f/2.0, shallow depth of field, ISO 100. "
        "Composition: centered subject with negative space for ethereal background drift; straight visual horizon, submerged structure lines guide eye into dreamlike perspective. "
)
