...
class StabilityImageEditor:
    ...
    def feather_mask(self, mask: Image.Image, radius: int = 12) -> Image.Image:
        ...
        """
        # Save black region to restore later
        black = mask.point(lambda p: 255 if p == 0 else 0)
        # Blur mask to soften 255→0 transition
        blurred = mask.filter(ImageFilter.GaussianBlur(radius=radius))
        # Restore exact blacks
        blurred.paste(0, mask=black)
        return blurred
...
def example_morne_background_and_face(
    ...
    final_mask = editor.feather_mask(final_mask, radius=feather_radius)
    if save_debug_mask:
    ...
    success, saved_path = editor.edit_image(
        ...
        mask=final_mask,
        prompt=prompt,
        negative_prompt="text, letters, words, typography, watermark, logo, brand, color shift, discoloration, crowds, artifacts, extra text",
