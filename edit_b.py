...
class StabilityImageEditor:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.relight_url = "https://api.stability.ai/v2beta/stable-image/edit/replace-background-and-relight"
        self.headers = {
    ...
    def replace_background_and_relight(
        ...
        light_source_direction: Optional[str] = None,
        preserve_original_subject: Optional[float] = None,
        original_background_depth: Optional[float] = None,
        output_format: str = "png",
        run_dir: Optional[str] = None,
    ) -> Tuple[bool, str]:
        """
        Replace Background and Relight (async JSON response with id).
        Docs: https://platform.stability.ai/docs/api-reference#tag/Edit/paths/~1v2beta~1stable-image~1edit~1replace-background-and-relight/post
        """
        assert background_prompt or background_reference_path, "Provide background_prompt or background_reference_path"

        files = {
            "subject_image": ("subject.png", open(subject_image_path, "rb"), "image/png"),
        }
        if background_reference_path:
            files["background_reference"] = ("background.png", open(background_reference_path, "rb"), "image/png")
        ...
        if negative_prompt:
            data["negative_prompt"] = negative_prompt
        data["keep_original_background"] = "true" if keep_original_background else "false"
        if light_source_direction:
            data["light_source_direction"] = light_source_direction
        if preserve_original_subject is not None:
            data["preserve_original_subject"] = str(preserve_original_subject)
        if original_background_depth is not None:
            data["original_background_depth"] = str(original_background_depth)
        ...
        try:
            ...
            target_dir = Path(run_dir) if run_dir else Path(".")
            target_dir.mkdir(parents=True, exist_ok=True)
            stamped_name = Path(self.timestamped_filename("relight_response.json")).name
            stamped_path = target_dir / stamped_name
            with open(stamped_path, "wb") as f:
                f.write(response.content)
            ok = response.status_code in (200, 202)
            return ok, str(stamped_path)
        except Exception as e:
            print("Relight call failed:", e)
            return False, ""

    @staticmethod
    def extract_generation_id_from_json(json_path: str) -> Optional[str]:
        try:
    ...
    def fetch_result(
        ...
        generation_id: str,
        run_dir: Optional[str] = None,
        output_name: str = "relight_result.png",
        max_attempts: int = 60,
        sleep_seconds: int = 10,
        wait_forever: bool = True,
    ) -> Tuple[bool, str]:
        url = f"https://api.stability.ai/v2beta/results/{generation_id}"
        attempt = 0
        while True:
            resp = requests.get(
                url,
                headers={"authorization": self.headers["authorization"], "accept": "*/*"},
            )
            if resp.status_code == 202:
                print("Result not ready yet. Waiting", sleep_seconds, "seconds...")
                time.sleep(sleep_seconds)
                attempt += 1
                if not wait_forever and attempt >= max_attempts:
                    return False, ""
                continue
            if resp.status_code == 200:
                target_dir = Path(run_dir) if run_dir else Path(".")
                target_dir.mkdir(parents=True, exist_ok=True)
                stamped = Path(self.timestamped_filename(output_name)).name
                out_path = target_dir / stamped
            ...
            try:
                ...
                err_json = {"status": resp.status_code, "detail": "non-json error"}
            target_dir = Path(run_dir) if run_dir else Path(".")
            (target_dir / "error_result.json").write_text(json.dumps(err_json, indent=2))
            return False, ""
        return False, ""
...
def example_replace_background_and_relight(
    background_prompt: str = "summit of Le Morne Brabant, Mauritius, cinematic light",
    negative_prompt: str = "crowds, artifacts, extra text",
    foreground_prompt: Optional[str] = "handsome 35-year-old male, white polo, natural skin tones",
    keep_original_background: bool = False,
    light_source_direction: Optional[str] = "above",
):
    editor = StabilityImageEditor()
    run_dir = StabilityImageEditor.create_run_directory()
    try:
        shutil.copy(DEFAULT_IMAGE_PATH, str(Path(run_dir) / Path(DEFAULT_IMAGE_PATH).name))
    except Exception:
        pass
    ok, saved = editor.replace_background_and_relight(
        subject_image_path=DEFAULT_IMAGE_PATH,
    ...
    gen_id = StabilityImageEditor.extract_generation_id_from_json(saved) if ok else None
    result_ok, result_path = (False, "")
    if gen_id:
        result_ok, result_path = editor.fetch_result(
            gen_id, run_dir=run_dir, output_name="relight_result.png", sleep_seconds=10, wait_forever=True
        )

    StabilityImageEditor.write_log(
        run_dir,
        {
            "api_endpoint": editor.relight_url,
            "background_prompt": background_prompt,
            "foreground_prompt": foreground_prompt,
            "negative_prompt": negative_prompt,
            "keep_original_background": keep_original_background,
            "light_source_direction": light_source_direction,
            "subject": DEFAULT_IMAGE_PATH,
...
if __name__ == "__main__":
    ...
    # combined_example()
    example_replace_background_and_relight()